...
np.set_printoptions(precision=3, threshold=1024)
...
class Learner():
    def __init__(self,model,args,trainloader,testloader, use_cuda, attacker):
        self.model=model
    ...
    def learn(self):
        ...
        for epoch in range(0, self.args.epochs):
            ...
            self.train(self.model, self.attacker, epoch)
    ...
    def train(self, model, attacker, epoch):
        model.train()
            ...
            for task_idx in range(1+self.args.sess):
                ...
                if(len(idx)>0):
                    ...
                    for kr in range(self.args.r):
                        _, class_outputs, sa_w = model(class_inputs)
                        ...
                        if attacker != None:
                            ...
                            sa_b = one - torch.mul(sa_w,sa_w)
                            adv = attacker.perturb(class_inputs, class_targets, sa_b)
                            _, class_outputs, sa_w = model(adv)
                            class_tar_ce=class_targets_one_hot.clone()
                            class_pre_ce=class_outputs.clone()
                    ...
                    for i,(p,q) in enumerate(zip(model.parameters(), model_base.parameters())):
                        ...
                        mean[i].append(self.get_corr(p.data,q.data))
                    num_updates += 1
                    np_mean = np.array(mean)
                    np_mean = np_mean * np_mean +0.85

            for i,(p,q) in enumerate(zip(model.parameters(), model_base.parameters())):
                alpha = np.exp(-self.args.beta*((1.0*self.args.sess)/self.args.num_task))
                ll = torch.stack(reptile_grads[i])
                summary = 0
                for j in range(np_mean[i].shape[0]):
                    summary = summary + np_mean[i][j]/np.sum(np_mean[i]) * ll[j]
                p.data = summary*(alpha) + (1-alpha)* q.data
    ...
    def test(self, model):
        ...
        for batch_idx, (inputs, targets) in enumerate(self.testloader):
            ...
            outputs2, outputs, sa_w = model(inputs)
    ...
    def meta_test(self, model, memory, inc_dataset):
        ...
        for task_idx in range(self.args.sess+1):
            ...
            if(self.args.sess!=0):
                for ep in range(1):
                    ...
                    for batch_idx, (inputs, targets) in enumerate(meta_loader):
                        ...
                        _, outputs, sa_w = meta_model(inputs)
            ...
            for cl in range(self.args.class_per_task):
                ...
                loader = inc_dataset.get_custom_loader_class([class_idx], mode="test", batch_size=10) #单独class
            ...
            for batch_idx, (inputs, targets) in enumerate(self.testloader):
                ...
                _, outputs, sa_w = meta_model(inputs)
                outputs_base, _, sa_w = self.model(inputs)
                task_ids = outputs
        ...
        for i in range(self.args.sess+1):
            ...
            for j in range(self.args.class_per_task):
                try:
                    ...
                    pass
        print("\n".join([str(int(acc_task[k])).format(".4f") for k in acc_task.keys()]) )    
        print(class_acc)
    ...
    def get_corr(self, fake_Y, Y):
                ...
                corr = torch.sum(torch.abs(Y - fake_Y)) 
                return corr
