...
class Learner():
            ...
            for task_idx in range(1+self.args.sess):
                ...
                if(len(idx)>0):
                    ...
                    for kr in range(self.args.r):
                        ...
                        loss = F.binary_cross_entropy_with_logits(class_pre_ce[:, ai:bi], class_tar_ce[:, ai:bi]) #task内损失函数，class输出和标签的BCE
    ...
    def test(self, model):
        ...
        for batch_idx, (inputs, targets) in enumerate(self.testloader):
            ...
            targets_one_hot = torch.FloatTensor(inputs.shape[0], self.args.num_class) #100*100
            targets_one_hot.zero_()
            targets_one_hot.scatter_(1, targets[:,None], 1)
